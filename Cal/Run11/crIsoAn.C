...
//using cross-ratio method with sin(phi) fit
  Int_t crIsoAn( Float_t elow = 48, Float_t ehigh = 53, Float_t mlow = 0.00, Float_t mhigh = 0.20, Int_t fillnum = 15315, int mode = 1, char* infile="/star/u/zwzhu/01zwzhu/Run11/byfill/ipi0jetcor_stfms_Fill15315.root" ){
...
//        }
	...
	sprintf(resultname,"IsoAN2_BKG_Fill%d_mass%.2fto%.2fGeV_E%.1fto%.1fGeV_mode%d.txt",fillnum,mlow,mhigh,elow,ehigh,mode);
	sprintf(resultname2,"IsoANy2_BKG_Fill%d_mass%.2fto%.2fGeV_E%.1fto%.1fGeV_mode%d.txt",fillnum,mlow,mhigh,elow,ehigh,mode);
...
//    if (mode==0)    sprintf(trigselect,"(TrigBits&%d)&&Corr==1&&Zem<0.9 &&catagory>=1&&CjetPt>2&&TMath::Abs(vertexzyuxi)<68&&pipt>2",TrigBitsSelect);
//    if (mode==1)    sprintf(trigselect,"(TrigBits&%d)&&Corr==1&&Zem>0.98&&catagory>=1&&CjetPt>2&&TMath::Abs(vertexzyuxi)<68&&pipt>2",TrigBitsSelect);// for BKG mode, cata>=1
/*
 if (mode==0)  {
	sprintf(trigselect,"Corr==1&&Zem<0.9&&catagory>=1&&TMath::Abs(vertexzyuxi+0)<68&&pipt>2&&((TrigBits&%d&&pipt>1.84)||(TrigBits&%d&&pipt>2.875&&(phid1<9.1||phid2<9.1))||(TrigBits&%d&&pipt>2.05&&(phid1>9.1&&phid2>9.1))||(TrigBits&%d&&pipt>0.92)||(TrigBits&%d&&pipt>1.61)||(TrigBits&%d&&pipt>1.72)||(TrigBits&%d&&pipt>2.53))",JP1,JP2,JP2,SmlBd1,SmlBd2,LrgBd1,LrgBd2);
}
if (mode==1)  {
	sprintf(trigselect,"Corr==1&&Zem>0.98&&catagory>=1&&TMath::Abs(vertexzyuxi+0)<68&&pipt>2&&((TrigBits&%d&&pipt>1.84)||(TrigBits&%d&&pipt>2.875&&(phid1<9.1||phid2<9.1))||(TrigBits&%d&&pipt>2.05&&(phid1>9.1&&phid2>9.1))||(TrigBits&%d&&pipt>0.92)||(TrigBits&%d&&pipt>1.61)||(TrigBits&%d&&pipt>1.72)||(TrigBits&%d&&pipt>2.53))",JP1,JP2,JP2,SmlBd1,SmlBd2,LrgBd1,LrgBd2);
}
*/
// 20200427
	 if (mode==0)  {
sprintf(trigselect,"Corr==1&&Zem<0.9&&catagory>=1&&TMath::Abs(vertexzyuxi+0)<68&&pipt>2&&((TrigBits&%d&&pipt>2.7)||(TrigBits&%d&&pipt>4.3&&(phid1<9.1||phid2<9.1))||(TrigBits&%d&&pipt>4.3&&(phid1>9.1&&phid2>9.1))||(TrigBits&%d&&pipt>1.6)||(TrigBits&%d&&pipt>2.7)||(TrigBits&%d&&pipt>2.7)||(TrigBits&%d&&pipt>4.3))",JP1,JP2,JP2,SmlBd1,SmlBd2,LrgBd1,LrgBd2);
}
if (mode==1)  {
	sprintf(trigselect,"Corr==1&&Zem>0.98&&catagory>=1&&TMath::Abs(vertexzyuxi+0)<68&&pipt>2&&((TrigBits&%d&&pipt>2.7)||(TrigBits&%d&&pipt>4.3&&(phid1<9.1||phid2<9.1))||(TrigBits&%d&&pipt>4.3&&(phid1>9.1&&phid2>9.1))||(TrigBits&%d&&pipt>1.6)||(TrigBits&%d&&pipt>2.7)||(TrigBits&%d&&pipt>2.7)||(TrigBits&%d&&pipt>4.3))",JP1,JP2,JP2,SmlBd1,SmlBd2,LrgBd1,LrgBd2);
}
	Char_t htitle[300];
	Char_t htitley[300];
	sprintf(htitle,"IsocrRawAsyFill%d_E%d_M%.2fto%.2fGeV_AllTrigger_mode%d",fillnum,ene,mlow,mhigh,mode);
	sprintf(htitley,"Isoy_crRawAsyFill%d_E%d_M%.2fto%.2fGeV_AllTrigger_mode%d",fillnum,ene,mlow,mhigh,mode);
...
//	sprintf(filename,"/star/u/zwzhu/01zwzhu/Run11/byfill/ipi0jetcor_stfms_Fill%d.root",fillnum,fillnum);
...
//all pions
	sprintf(cutbup,  "pim>%.2f&&pim<%.2f&&TMath::Abs(pie-%.1f)<%f&&piz<%.1f&&pieta>%.1f&&pieta<%.1f&&%s&&%s",mlow,mhigh,ene,enew,pizcut,pietalow,pietahigh,bup,trigselect);
	sprintf(cutbdown,"pim>%.2f&&pim<%.2f&&TMath::Abs(pie-%.1f)<%f&&piz<%.1f&&pieta>%.1f&&pieta<%.1f&&%s&&%s",mlow,mhigh,ene,enew,pizcut,pietalow,pietahigh,bdown,trigselect);
	sprintf(cutyup,  "pim>%.2f&&pim<%.2f&&TMath::Abs(pie-%.1f)<%f&&piz<%.1f&&pieta>%.1f&&pieta<%.1f&&%s&&%s",mlow,mhigh,ene,enew,pizcut,pietalow,pietahigh,yup,trigselect);
	sprintf(cutydown,"pim>%.2f&&pim<%.2f&&TMath::Abs(pie-%.1f)<%f&&piz<%.1f&&pieta>%.1f&&pieta<%.1f&&%s&&%s",mlow,mhigh,ene,enew,pizcut,pietalow,pietahigh,ydown,trigselect);
...
//	t1->Project(hbdtitle,"piphi",cutbdown);
	t1->Project(hbutitle,"piphi",cutbup); //plot yield sorted by blue beam polarizations
	t1->Project(hbdtitle,"piphi",cutbdown);
	t1->Project(hyutitle,"piphi",cutyup); //plot yield sorted by blue beam polarizations
	t1->Project(hydtitle,"piphi",cutydown);
...
//calculate raw cross-ratio asymmetry vs sin(phi)
	...
	for(Int_t ibin = 1; ibin <= nphibin/2; ibin++){
		...
		cout<<"ibin="<<ibin<<" bup="<<totbup[ibin-1]<<" bdown+pi="<<totbup[ibin-1+nphibin/2]<<" bup+pi="<<totbup[ibin-1+nphibin/2]<<" bdown="<<totbdown[ibin-1]<<endl;
...
//	fit->SetParameter(0,relumi);
        h1asyphi->Fit(fit,"R+");
	Char_t gifname[200];
        sprintf(gifname,"gifs/IsoAN/crasyxfblueFill%d_7ebins_E%.1fGeV_M%.2ftoM%.2fGeV_AllTrigger_mode%d.gif",fillnum,ene,mlow,mhigh,mode);
	Char_t coutname[200];
        sprintf(coutname,"coutfile/IsoAN/crasyxfblueFill%d_7ebins_E%.1fGeV_M%.2ftoM%.2fGeV_AllTrigger_mode%d.C",fillnum,ene,mlow,mhigh,mode);
...
//	c1->Print("tmp.ps");
	...
	for(Int_t ibin = 1; ibin <= nphibin/2; ibin++){
		...
		//rawasybinerr = TMath::Sqrt( 1/(totbup[ibin-1]+totbdown[ibin-1]) + TMath::Power(totbup[ibin-1]-totbdown[ibin-1],2)/TMath::Power(totbup[ibin-1]+totbdown[ibin-1],3 ));
	       if (invalid ==0) {	
			   cout<<"valid yellow denominator"<<endl;}
...
//	fit->SetParameter(0,relumi);
        h2asyphi->Fit(fit,"R+");
	Char_t gifname2[200];
        sprintf(gifname2,"gifs/IsoANy/y_crasyxfblueFill%d_7ebins_E%.1fGeV_M%.2ftoM%.2fGeV_AllTrigger_mode%d.gif",fillnum,ene,mlow,mhigh,mode);
	Char_t coutname2[200];
        sprintf(coutname2,"coutfile/IsoANy/y_crasyxfblueFill%d_7ebins_E%.1fGeV_M%.2ftoM%.2fGeV_AllTrigger_mode%d.C",fillnum,ene,mlow,mhigh,mode);
