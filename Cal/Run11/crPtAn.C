...
//using cross-ratio method with sin(phi) fit
  Int_t crPtAn( Float_t elow = 38, Float_t ehigh = 43, Float_t mlow = 0.20, Float_t mhigh = 0.25,Float_t ptlow = 1.0, Float_t pthigh = 2.0, Int_t fillnum = 15315, char* infile="/star/u/zwzhu/01zwzhu/Run11/byfill/ipi0jetcor_stfms_Fill15315.root" ){
...
//        }
	...
	sprintf(resultname, "PtAN2_Fill%d_mass%.2fto%.2fGeV_E%.1fto%.1f_Pt%.1fto%.1fGeV.txt",fillnum,mlow,mhigh,elow,ehigh,ptlow,pthigh);
	sprintf(resultname2,"PtANy2_Fill%d_mass%.2fto%.2fGeV_E%.1fto%.1f_Pt%.1fto%.1fGeV.txt",fillnum,mlow,mhigh,elow,ehigh,ptlow,pthigh);
	...
	Char_t bdown[55] = "(spinbit==5||spinbit==6)";
	Char_t bup[55] = "(spinbit==9||spinbit==10)";
	Char_t ydown[55] = "(spinbit==5||spinbit==9)";
	Char_t yup[55] = "(spinbit==10||spinbit==6)";
...
//	Int_t TrigBitsSelect = JP2;
 ...
 //       sprintf(trigselect,"(TrigBits&%d&&Corr==0&&TMath::Abs(vertexzyuxi+0)<68)&&pipt>%.1f&&pipt<%.1f",TrigBitsSelect,ptlow,pthigh);
//sprintf(trigselect,"Corr==0&&TMath::Abs(vertexzyuxi+0)<68&&pipt>%.1f&&pipt<%.1f&&((TrigBits&%d&&pipt>1.84)||(TrigBits&%d&&pipt>2.875&&(phid1<9.1||phid2<9.1))||(TrigBits&%d&&pipt>2.05&&(phid1>9.1&&phid2>9.1))||(TrigBits&%d&&pipt>0.92)||(TrigBits&%d&&pipt>1.61)||(TrigBits&%d&&pipt>1.72)||(TrigBits&%d&&pipt>2.53))",ptlow,pthigh,JP1,JP2,JP2,SmlBd1,SmlBd2,LrgBd1,LrgBd2);
//20200427
sprintf(trigselect,"Corr==0&&TMath::Abs(vertexzyuxi+0)<68&&pipt>%.1f&&pipt<%.1f&&((TrigBits&%d&&pipt>2.7)||(TrigBits&%d&&pipt>4.3&&(phid1<9.1||phid2<9.1))||(TrigBits&%d&&pipt>4.3&&(phid1>9.1&&phid2>9.1))||(TrigBits&%d&&pipt>1.6)||(TrigBits&%d&&pipt>2.7)||(TrigBits&%d&&pipt>2.7)||(TrigBits&%d&&pipt>4.3))",ptlow,pthigh,JP1,JP2,JP2,SmlBd1,SmlBd2,LrgBd1,LrgBd2);
	Char_t htitle[200];
	Char_t htitley[200];
...
//	fit->SetParameter(0,relumi);
        h1asyphi->Fit(fit,"R+");
	Char_t gifname[200];
        sprintf(gifname,"AN_Fill%d_E%.1fGeV_M%.2ftoM%.2fGeV_AllTrigger.png",fillnum,ene,mlow,mhigh);
	Char_t coutname[200];
        sprintf(coutname,"AN_Fill%d_E%.1fGeV_M%.2ftoM%.2fGeV_AllTrigger.C",fillnum,ene,mlow,mhigh);
...
//	fit->SetParameter(0,relumi);
        h2asyphi->Fit(fit,"R+");
	Char_t gifname2[200];
        sprintf(gifname2,"ANy_crasyxfblueFill%d_7ebins_E%.1fGeV_M%.2ftoM%.2fGeV_AllTrigger.png",fillnum,ene,mlow,mhigh);
	Char_t coutname2[200];
        sprintf(coutname2,"ANy_crasyxfblueFill%d_7ebins_E%.1fGeV_M%.2ftoM%.2fGeV_AllTrigger.C",fillnum,ene,mlow,mhigh);
