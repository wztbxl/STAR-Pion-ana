...
//using cross-ratio method with sin(phi) fit
  Int_t crjetAn( Float_t elow = 48, Float_t ehigh = 53, Float_t mlow = 1.00, Float_t mhigh = 1.00, Int_t fillnum = 15315,char* infile="/star/u/zwzhu/01zwzhu/Run11/byfill/ipi0jetcor_stfms_Fill15315.root" ){
...
double uee3   =       2.73998;   
	...
	//Int_t TrigBitsSelect = JP+HiTow+clbit;
        ...
        //sprintf(trigselect,"(TrigBits&%d&&TMath::Abs((vertexzyuxi+0)<68))",TrigBitsSelect);
        Char_t ptnew[400];
        ...
        sprintf(trigselect,"TMath::Abs(vertexzyuxi+0)<68&&fjetntow>0&&((TrigBits&%d&&fjpT>2.7)||(TrigBits&%d&&fjpT>4.3))",JP1,JP2);
