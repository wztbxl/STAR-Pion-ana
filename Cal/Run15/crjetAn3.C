...
#include<fstream>//FOR Mul >2 
using namespace std;

//calculate asy. for pi0 side band, used as background asymmetries
//using cross-ratio method with sin(phi) fit
  Int_t crjetAn3( Float_t elow = 48, Float_t ehigh = 53, Float_t mlow = 1.00, Float_t mhigh = 1.00, Int_t fillnum = 18790,char* infile="/star/u/zwzhu/01zwzhu/Run15/byfill/ipi0jetcor_stfms_Fill18790.root" ){
...
//        }
	...
	sprintf(resultname,"jetAnM3JP_Fill%d_mass%.2fto%.2fGeV_E%.1fto%.1fGeV.txt",fillnum,mlow,mhigh,elow,ehigh);
	sprintf(resultname2,"jetAnyM3JP_Fill%d_mass%.2fto%.2fGeV_E%.1fto%.1fGeV.txt",fillnum,mlow,mhigh,elow,ehigh);
...
//a        sprintf(trigselect,"(TrigBits&%d&&TMath::Abs((vertexzyuxi+18.4)<93.4))&&fjetntow>2&&bbcm>0.5&&tofm>2.5",TrigBitsSelect);
     //   sprintf(trigselect,"(TrigBits&%d&&TMath::Abs((vertexzyuxi+18.4)<93.4))&&fjetntow>2",TrigBitsSelect);
        Char_t ptnew[400];
	...
	sprintf(Rof,"bbcm>0.5 && tofm>2.5");
        sprintf(trigselect,"TMath::Abs(vertexzyuxi+18.4)<93.4&&fjetntow>2&&((TrigBits&%d&&fjpT>1.84)||(TrigBits&%d&&fjpT>2.765)||(TrigBits&%d&&fjpT>3.68))&&%s",JP0,JP1,JP2,Rof);
