...
//using cross-ratio method with sin(phi) fit
  Int_t crColl( Float_t zlow = 0.7, Float_t zhigh = 0.8, Float_t mlow = 0.20, Float_t mhigh = 0.30, Int_t fillnum = 18761, Float_t Jtlow = 0.20 , Float_t Jthigh = 9.99 ,char* infile="/star/u/zwzhu/01zwzhu/Run15/byfill/ipi0jetcor_stfms_Fill18761.root"){
	...
	Float_t pietalowJ = 2.9;
        Float_t pietahighJ = 3.8;
	Float_t pizcut = 0.7;
	float ecut=10;
	Float_t Jtmin= Jtlow;
...
//	Int_t TrigBitsSelect = JP2;
	Int_t TrigBitsSelect = JP+HiTow+clbit;
        Char_t trigselect[300];
        sprintf(trigselect,"(TrigBits&%d)&&TMath::Abs(vertexzyuxi+18.4)<93.4&&CjetEta>%.1f&&CjetEta<%.1f",TrigBitsSelect,pietalowJ,pietahighJ);
