...
//using cross-ratio method with sin(phi) fit
  Int_t crIsoAn( Float_t elow = 18, Float_t ehigh = 23, Float_t mlow = 0.20, Float_t mhigh = 0.30, Int_t fillnum = 18854, int mode = 1,  char* filename1 = "/star/u/zwzhu/01zwzhu/Run15/byfill/ipi0jetcor_stfms_Fill18854.root" ){
...
//        }
	...
	sprintf(resultname,"IsoAN_BKG_Fill%d_mass%.2fto%.2fGeV_E%.1fto%.1fGeV_mode%d.txt",fillnum,mlow,mhigh,elow,ehigh,mode);
	sprintf(resultname2,"IsoANy_BKG_Fill%d_mass%.2fto%.2fGeV_E%.1fto%.1fGeV_mode%d.txt",fillnum,mlow,mhigh,elow,ehigh,mode);
...
//   if (mode==0)    sprintf(trigselect,"(TrigBits&%d)&&Corr==1&&Zem<0.9 &&catagory==1&&CjetPt>2&&TMath::Abs(vertexzyuxi+36)<90&&pipt>2",TrigBitsSelect);
//    if (mode==1)    sprintf(trigselect,"(TrigBits&%d)&&Corr==1&&Zem>0.98&&catagory==1&&CjetPt>2&&TMath::Abs(vertexzyuxi+36)<90&&pipt>2",TrigBitsSelect);
    if (mode==0)   sprintf(trigselect,"Corr==1&&Zem<0.9 &&catagory>=1&&TMath::Abs(vertexzyuxi+18.4)<93.4&&pipt>2&&((TrigBits&%d&&pipt>1.84)||(TrigBits&%d&&pipt>2.765)||(TrigBits&%d&&pipt>3.68)||(TrigBits&%d&&pipt>1.26)||(TrigBits&%d&&pipt>1.84)||(TrigBits&%d&&pipt>2.2)||(TrigBits&%d&&pipt>2.53)||(TrigBits&%d&&pipt>1.26)||(TrigBits&%d&&pipt>1.84)||(TrigBits&%d&&pipt>2.76))",JP0,JP1,JP2,SmlBd0,SmlBd1,SmlBd2,SmlBd22,LrgBd0,LrgBd1,LrgBd2);
    if (mode==1)   sprintf(trigselect,"Corr==1&&Zem>0.98 &&catagory>=1&&TMath::Abs(vertexzyuxi+18.4)<93.4&&pipt>2&&((TrigBits&%d&&pipt>1.84)||(TrigBits&%d&&pipt>2.765)||(TrigBits&%d&&pipt>3.68)||(TrigBits&%d&&pipt>1.26)||(TrigBits&%d&&pipt>1.84)||(TrigBits&%d&&pipt>2.2)||(TrigBits&%d&&pipt>2.53)||(TrigBits&%d&&pipt>1.26)||(TrigBits&%d&&pipt>1.84)||(TrigBits&%d&&pipt>2.76))",JP0,JP1,JP2,SmlBd0,SmlBd1,SmlBd2,SmlBd22,LrgBd0,LrgBd1,LrgBd2);

	Char_t htitle[300];
	Char_t htitley[300];
	sprintf(htitle,"Iso_crRawAsyFill%d_E%d_M%.2fto%.2fGeV_AllTrigger_mode%d",fillnum,ene,mlow,mhigh,mode);
...
//calculate raw cross-ratio asymmetry vs sin(phi)
	...
	for(Int_t ibin = 1; ibin <= nphibin/2; ibin++){
		...
		cout<<"ibin="<<ibin<<" bup="<<totbup[ibin-1]<<" bdown+pi="<<totbdown[ibin-1+nphibin/2]<<" bup+pi="<<totbup[ibin-1+nphibin/2]<<" bdown="<<totbdown[ibin-1]<<endl;
