...
public:
  ...
  StJetParticle* addParticle(StJetParticle* particle) { mParticles.Add((TObject*)particle); return (StJetParticle*)mParticles.Last(); }
    ...
    void setUedensity( float density){ mUedensity = density; }
	    void setUeMul( float mul){ mUeMul = mul; }
		    float uedensity() const { return mUedensity; }
			    float ueMul() const { return mUeMul; }
