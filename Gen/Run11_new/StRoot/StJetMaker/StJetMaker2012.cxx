...
// 			    jetbranch->anapars->randomSelectorSeed);
      ...
      // Vertex loop
    //  for (int iVertex = 0; iVertex < tpc.numberOfVertices(); ++iVertex) {//not for fms
	tpc.setVertexIndex(0);
	...
	for (StJetFinder::JetList::const_iterator iProtoJet = protojets.begin(); iProtoJet != protojets.end(); ++iProtoJet){
	  ...
	  for(size_t ueBranch = 0; ueBranch < mJetUeBranches.size(); ueBranch++){
	    ...
	    double radius = jetuebranch->uePars->coneRadius();
	                 float mul;
	    double uedensity = addJetUe(particles, (jetuebranch->eventUe).at(iBranch), *iProtoJet, radius, mul);
	    currentjet->setUeDensity(jetuebranch->name, uedensity);
	    currentjet->setUedensity(uedensity);
	    currentjet->setUeMul( mul);
...
double StJetMaker2012::addJetUe(StProtoJet::FourVecList particles, StUeOffAxisConesEvent *ueEvent, const StProtoJet &jet, double radius, float & mul)
{
...
//  cout<<"add new jet to underlying event"<<endl;
  ...
  mul = (static_cast<float>(number[0])+static_cast<float>(number[1]))/2;
    cout<< "UE E="<<pt[0]<<" "<<pt[1]<<" N="<<number[0]<<" "<<number[1]<<" mul"<<" "<<mul<<" area="<<jarea<<endl;
