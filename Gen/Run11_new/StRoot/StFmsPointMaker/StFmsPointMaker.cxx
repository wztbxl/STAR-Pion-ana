// $Id: StFmsPointMaker.cxx,v 1.1 2020/08/01 23:50:17 zwzhu Exp $
//
...
namespace {
  ...
  StLorentzVectorD compute4Momentum(const StThreeVectorD& xyz, double energy) {
    StThreeVectorD mom3 = xyz.unit() * energy;  // Momentum vector with m = 0
    return StLorentzVectorD(mom3, energy);
  }
}  // unnamed namespace
...
StFmsPointMaker::StFmsPointMaker(const char* name)
    : StMaker(name), mObjectCount(0), mMaxEnergySum(255.0), mReadMuDst(0), 
      ...
      mCategorizationAlgo(0), mScaleShowerShape(0),mVertexZ(1) {
		 // skew 18.5/12.5
        EDepCorrection1=new TF1("EDepCorrection1","1.17731+ 0.915*x -0.000754*x*x",0,120); //FMS energy dependent correction
        EDepCorrection2=new TF1("EDepCorrection2","0.909 + 0.987*x -0.00166*x*x",0,120); //FMS energy dependent correction
...
bool StFmsPointMaker::processTowerCluster(
 ...
 // Cluster locations are in column-row grid coordinates so convert to cm and get STAR xyz
  ...
  // shift the z position according to BBC info, although it is not relevant to fitting
  //if(vertexz>0) cout<<"------------------vertexz="<<vertexz<<"  D=   "<<xyz.z() - vertexz<<endl;
  xyz.setZ( xyz.z() - vertexz  );
  cluster->setFourMomentum(compute4Momentum(xyz, cluster->energy()));
  // Save photons reconstructed from this cluster
	double insertE;
       if (towerCluster->photons().size() ==1 )    insertE=cluster->energy();
       if (towerCluster->photons().size() !=1 )    insertE=-1;
  for (UInt_t np = 0; np < towerCluster->photons().size(); np++) {
      StFmsPoint* point = makeFmsPoint(towerCluster->photons()[np], detectorId, insertE );
      ...
      point->setChi2(towerCluster->chiSquare());
...
StFmsPoint* StFmsPointMaker::makeFmsPoint(
    const FMSCluster::StFmsFittedPhoton& photon, const int detectorId , double clusterE) {
  StFmsPoint* point = new StFmsPoint;
  double x=photon.x; //photon xy is in detector local coordinate in [cm]
  double y=photon.y;
  int det=detectorId;
  if(mMergeSmallToLarge>0){
 ...
 point->setEnergy(EDep(photon.energy,det));
  ...
  xyz.setZ( 735.45 - vertexz  );
  //bypass the Db function of Z in order to add vetexz
  point->setXYZ(xyz);  //This is in STAR global coordinate
  point->setFourMomentum(compute4Momentum(xyz, point->energy()));
  //bypass the digit problem by local FMSpoint.cxx
  point->setdX(compute4Momentum(xyz, point->energy()).x());
  point->setdY(compute4Momentum(xyz, point->energy()).y());
  point->setdZ(compute4Momentum(xyz, point->energy()).z());
  point->setdE(compute4Momentum(xyz, point->energy()).e());
  ...
  if(x<8.0*lwx && y>9.0*lwy && y<25.0*lwy)   { x=x/wx-floor(x/wx); y=y/wy-floor(y/wy);}//small cell
