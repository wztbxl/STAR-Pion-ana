// $Id: StFmsClusterFinder.cxx,v 1.1 2020/08/01 23:50:21 zwzhu Exp $
//
...
// cout<<" towerfilter E="<<tower->hit()->energy()<<TOWER_E_THRESHOLD<<endl;
...
/*  for (auto i = belowThreshold.begin(); i != belowThreshold.end(); ++i) {
	cout<<" Etow="<< (*i)->hit()->energy()<<endl;
	if ((*i)->hit()->energy()<0.1) cout<<"      BUDELIAO     "<<endl;
  }
*/ //see tower list
...
int StFmsClusterFinder::findClusters(TowerList* towers, ClusterList* clusters, int detectorId) {
  ...
  TowerList neighbors;  // List of non-peak towers in clusters
   int clusternum=locateClusterSeeds(towers, &neighbors, clusters);
//cout<<clusternum<<"    check slunum  "<<endl;
  ...
  TObjArray valleys(20);  // Stores towers equidistant between seeds //zzw  16->20
...
// cout<<"  find clu re nb=   " <<count<<endl;
...
//	cout<<" check  tower nb =     "<<count<<endl;
...
//cout<<" match with cl "<< association->clusters()->front()->towers().front()->hit()->energy()<<endl;
