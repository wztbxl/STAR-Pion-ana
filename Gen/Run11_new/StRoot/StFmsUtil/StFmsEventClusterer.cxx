// $Id: StFmsEventClusterer.cxx,v 1.1 2020/08/01 23:50:21 zwzhu Exp $
//
...
int smallmergetolarge=0;
int showershapewithangle=1;
...
struct IsBadCluster {
  ...
  bool operator()(const fms::ClusterList::value_type& cluster) const {
	  ...
	  //cout << Form("Removing cluster because E=%f <= %f (NTower=%d)",cluster->cluster()->energy(),energy,cluster->towers().size()) << endl;
      }
...
struct TwoPhotonFitParameters {
  ...
  TwoPhotonFitParameters(const std::vector<double>& xyWidth,
                         const fms::StFmsTowerCluster* towerCluster, const double ww, double vertexZ) {
    const double x = xyWidth.at(0);
    const double y = xyWidth.at(1);
	double w1=0;
	double w2=0;
        double adjust_PH2_START_FSIGMAMAX;
    if (ww<4) {w1=3.822; w2=3.822; 
    if ( smallmergetolarge>0 )      adjust_PH2_START_FSIGMAMAX=PH2_START_FSIGMAMAX/5.812*3.822;
	 else          adjust_PH2_START_FSIGMAMAX=PH2_START_FSIGMAMAX;}
    if (ww>4) {w1=ww; w2=ww;  adjust_PH2_START_FSIGMAMAX=PH2_START_FSIGMAMAX;}
    const auto cluster = towerCluster->cluster();
    start = std::array<double, 7>{ {
      PH2_START_NPH,
      x * cluster->x(),
      y * cluster->y(),
      adjust_PH2_START_FSIGMAMAX * x * cluster->sigmaMax()/2, // 1/2 factor in 12-18zzw
      0-towerCluster->thetaAxis(),   //ZZW  due to coordinate change
	0,
      cluster->energy(),
    } };
	...
	std::max(0.1345*2/(cluster->energy())*(735-vertexZ) , 0.5*x ),
      start.at(4) - maxTheta,
      PH2_LOWER_5_F,
    ...
    upper = std::array<double, 7>{ { 
      ...
      std::min(PH2_UPPER_XMIN_F * (PH2_UPPER_XMIN_P0 - sigmaMaxE), PH2_UPPER_XMIN_LIMIT) * x*5,
      start.at(4) + maxTheta,
      PH2_UPPER_5_F,
...
//make it same to yuxi
    if( lower.at(3) >= start.at(3) ) {
      lower.at(3) = start.at(3) * 0.8 ; }
    if( upper.at(3) <= start.at(3) ) {
      upper.at(3) = start.at(3) * 1.5 ; }
...
namespace FMSCluster {
  StFmsEventClusterer::StFmsEventClusterer( //const StFmsGeometry* geometry,
					   ...
					   Int_t scaleShowerShape, StMuDst* muDst)
	...
	mTry1PhotonFitWhen2PhotonFitFailed(try1PhotonFit), mCategorizationAlgo(categorizationAlgo),
        mScaleShowerShape(scaleShowerShape),MuDst(muDst) { }
...
Bool_t StFmsEventClusterer::cluster(std::vector<StFmsTower>* towerList) {
  ...
  mFitter.reset(new StFmsClusterFitter(/*mGeometry,*/ mDetectorId,xw,yw,mScaleShowerShape,mFmsDbMaker, MuDst));


           unsigned short bbcTimeBin = MuDst->event()->bbcTriggerDetector().onlineTimeDifference();
            vertexz=0;
           if(bbcTimeBin!=0)  vertexz = 633.544 - 0.158*bbcTimeBin; //in cm
...
//    cout<<" intofuc tx="<<tower->x()<<" px="<<photon->x<<" ty="<<tower->y()<<endl;
if (showershapewithangle>0)  return photon->energy * mFitter->showerShapeFunction()->Eval(tower->x(),photon->x,tower->y(),photon->y);
if (showershapewithangle==0)  return photon->energy * mFitter->showerShapeFunction()->Eval(tower->x()-photon->x,tower->y()-photon->y);
...
Int_t StFmsEventClusterer::fitAmbiguousCluster(ClusterIter towerCluster) {
  ...
  if (chiSquare1Photon >= 5.) {
	  ...
	  else	cout<<" False False False"<<endl;
      }  // if
  }  // if
