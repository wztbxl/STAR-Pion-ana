// $Id: StFmsClusterFitter.cxx,v 1.1 2020/08/01 23:50:21 zwzhu Exp $
//
...
int mshowershapewithangle=2; // option
int ScaleShowerShape=1;
int msmallmergetolarge=0;
int mVertexZ=0;//discard vertex in fitting
...
void choose_fitParameters(int detID){
  ...
  if ( mshowershapewithangle==1 ) {
        if (detID>9.9) {//small cell
                for (int i=0 ; i<6 ; i++) {
                a1[i]=a1S[i];
                a2[i]=a2S[i];
                b1[i]=b1S[i];
...
void StFmsClusterFitter::SetMerge(int m){
     mMerge = m;
}
void StFmsClusterFitter::SetVertexZ(double z){
     vertex_z = z;
}
...
StFmsClusterFitter::StFmsClusterFitter( //const StFmsGeometry* geometry,
                                       Int_t detectorId, Float_t xw, Float_t yw, Int_t scaleShowerShape,StFmsDbMaker* mFmsDbMaker, StMuDst* muDst)
    : mdetectorId(detectorId),mScaleShowerShape(scaleShowerShape), mDb(mFmsDbMaker),mMinuit(3 * kMaxNPhotons + 1) {
  // Set tower (x, y) widths for this detector
  towerWidths.clear();
  towerWidths.push_back(xw);
  towerWidths.push_back(yw);
  clear_fitParameters();
  choose_fitParameters(mdetectorId);
  showerShapeFitFunction.SetParameters(fitParameters.data());
  mMinuit.SetPrintLevel(-1);  // Quiet, including suppression of warnings
  mMinuit.fLwarn = false ;  // Quiet, including suppression of warnings

  SetVertexZ( 0 );
  SetMerge(msmallmergetolarge );
}
...
//cout<<"nphoton=="<<nPhotons<<endl;
  ...
  for (Int_t i = 0; i < nPhotons; i++) {
    ...
    if (nPhotons==1) {mMinuit.FixParameter(3) ; } //cout<<"nPH=1 and fix E"<<endl; // fix Energy during 1P fit
    if (nPhotons==2) {mMinuit.FixParameter(6) ; mMinuit.FixParameter(3); }// fix Energy during 2P fit
...
mMinuit.SetPrintLevel(-1);
  ...
  if (mMinuit.GetStatus() == 0) {
    ...
    // Second photon
    double x1 = param[1] - cos(param[4]) * param[3] * (1 + param[5]) / 2.0;
    double y1 = param[2] + sin(param[4]) * param[3] * (1 + param[5]) / 2.0;
    double E1 = param[6] * (1 - param[5]) / 2.0;
    photons->emplace_back(x1, y1, E1);
...
Double_t StFmsClusterFitter::energyDepositionInTower(Double_t* xy,
 ...
 if(mshowershapewithangle==0){
  const double width = (parameters[0]>4 ? 5.812: 3.822) ;
 ...
 if(mshowershapewithangle>0){
	...
	Double_t ZcS[6] = {720.45,727.95,735.45,742.95,750.45,757.95};
      ...
      Double_t Zmax = -999;
        ...
        if (detId_SS <9.9) { yoff=98.8; Zmax=735.45; xoff=0.3;
                for(Int_t i = 0; i < 6; i++){
                        Zc[i]=ZcL[i];
                }
        }
        if (detId_SS >9.1) { yoff=46.5; Zmax=735.45; xoff=0.93;
                for(Int_t i = 0; i < 6; i++){
                        Zc[i]=ZcS[i];
                }
        }
	...
	if (msmallmergetolarge>0) yoff=98.8;  // large cells always 98.8
        Double_t tany = ( yoff - xy[3]) / Zmax; //use Zmax instead of lzz, 06/25/2012 Yuxi
        Double_t tanx = ( xy[1]- xoff) / Zmax; 	        //
        ...
        for(Int_t i = 0; i < 6; i++){
           ...
           Int_t istart = i*10;
           energy += energyDepositionInTowerSingleLayer(xy[0]-xc,xy[2]-yc,&para[istart])  *  para[istart+7]    ;
        }
  }
...
void StFmsClusterFitter::minimizationFunctionNPhoton(Int_t& npara,
  ...
  for (auto i = mTowers->begin(), e = mTowers->end(); i!=e; ++i){
    ...
    y *= towerWidths.at(1);
    */
	    fitParameters.front() = tower->w(); // for msmallmergetolarge=0 & =1 both
    Double_t x=tower->x();
    Double_t y=tower->y();
    Double_t detId=tower->hit()->detectorId();    // ZZ: this is done for each cell, setTower is done for leading cell.
	    fitParameters[10] = tower->w(); 
	    fitParameters[20] = tower->w(); 
	    fitParameters[30] = tower->w(); 
	    fitParameters[40] = tower->w(); 
	    fitParameters[50] = tower->w(); 
	
	if (ScaleShowerShape>0) { 
	 if(mTowers->front()->hit()->detectorId() <= 9){ 
	    fitParameters.at(4) = b11*1.5;fitParameters.at(5) = b12*1.5;fitParameters.at(6) = b13*1.5;
	    fitParameters.at(14) = b21*1.5;fitParameters.at(15) = b22*1.5;fitParameters.at(16) = b23*1.5;
	    fitParameters.at(24) = b31*1.5;fitParameters.at(25) = b32*1.5;fitParameters.at(26) = b33*1.5;
	    fitParameters.at(34) = b41*1.5;fitParameters.at(35) = b42*1.5;fitParameters.at(36) = b43*1.5;
	    fitParameters.at(44) = b51*1.5;fitParameters.at(45) = b52*1.5;fitParameters.at(46) = b53*1.5;
	    fitParameters.at(54) = b61*1.5;fitParameters.at(55) = b62*1.5;fitParameters.at(56) = b63*1.5;
	 }
	}
	...
	if(mshowershapewithangle>0 ){	expected += para[k + 3] * 
	    energyDepositionInTower(x , y ,para[k+1] , para[k+2], fitParameters.data(),detId,vertex_z,mMerge);  
	}
	if(mshowershapewithangle==0 ){	expected += para[k + 3] * 
...
//   const double err = 0.03 * measured *1  + 0.01;
...
void StFmsClusterFitter::minimizationFunction2Photon(Int_t& nparam,
  ...
  std::array<double, 7> oldParam{ {
    ...
    param[2] - sin(angle) * dgg * (1 - zgg) / 2.0,  // y 1  //ZZW due to coordinate change
...
void StFmsClusterFitter::setTowers(StFmsTowerCluster::Towers* towers) { 
    ...
    if(mshowershapewithangle==0 &&  ScaleShowerShape>0){
	if(mTowers->front()->hit()->detectorId() <= 9){ 
	    fitParameters.at(4) = fitParameters.at(4)*1.5;
	    fitParameters.at(5) = fitParameters.at(5)*1.5;
	    fitParameters.at(6) = fitParameters.at(6)*1.5;
	}
  ...
  for (auto i = mTowers->begin(), e = mTowers->end(); i!=e; ++i){
	...
	double ww=(mTowers->front()->w()>4 ? 5.812: 3.822);	
	fitParameters.front() = ww; // for msmallmergetolarge=0 & =1 both
 if(mshowershapewithangle>0){
   fitParameters[0]  = ww; 
   fitParameters[10] = ww; 
   fitParameters[20] = ww; 
   ...
   detId_SS=mTowers->front()->hit()->detectorId(); //leading cell ID
