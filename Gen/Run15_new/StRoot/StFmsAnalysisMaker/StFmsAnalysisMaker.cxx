...
#include "StJetMaker/StJetMaker2009.h"
...
StFmsAnalysisMaker::StFmsAnalysisMaker( const char* name, StJetMaker2012* jetmaker, int PP ):StMaker(name) { 
	Period=PP;
	mJetmaker = jetmaker; 
...
/*
	...
	*/
	...
	for (size_t iBranch = 0; iBranch < mJetmaker->mJetBranches.size(); ++iBranch) {
		mJetmaker->mJetBranches[iBranch]->jetfinder->Init();
...
//    tevent->Branch("fnPjet",&fmnPjet,"fnPjet/I");
    ...
    tevent->Branch("fnph",&mnph,"fnph/I");
            tevent->Branch("bbcm",&tBBCM,"b/bcm/b");
            tevent->Branch("tofm",&tTOFM,"tofm/b");
			tevent->Branch("Bunchid7bit",&mbunchid,"Bunchid7bit/I");
			tevent->Branch("bbccoincrate",&mbbccoincrate,"bbccoincrate/F");
	
//corelated pion
	tjetcor->Branch("Bunchid7bit",&mbunchid,"Bunchid7bit/I");
	tjetcor->Branch("bbccoincrate",&mbbccoincrate,"bbccoincrate/F");
            tjetcor->Branch("bbcm",&tBBCM,"b/bcm/b");
            tjetcor->Branch("tofm",&tTOFM,"tofm/b");
...
//fms jet
	tFmsjet->Branch("Bunchid7bit",&mbunchid,"Bunchid7bit/I");
	tFmsjet->Branch("bbccoincrate",&mbbccoincrate,"bbccoincrate/F");
            tFmsjet->Branch("bbcm",&tBBCM,"bbcm/I");
            tFmsjet->Branch("tofm",&tTOFM,"tofm/I");
	...
	tFmsjet->Branch("TrigBits",&mTrigBits,"TrigBits/I");  
 	tFmsjet->Branch("run",&mRnum,"run/I");
 	tFmsjet->Branch("ievt",&mievt,"ievt/I");
 	tFmsjet->Branch("spinbit",&mspinbit,"spinbit/I");
 	tFmsjet->Branch("vertexz",&mvertexz,"vertexz/F");
 	tFmsjet->Branch("vertexzyuxi",&mvertexzyuxi,"vertexzyuxi/F"); //for run15

 	tFmsjet->Branch("CollinsTag",&mCollinsTag,"CollinsTag/I");  // for associate with Collins
 	tFmsjet->Branch("LeadEjet",&mLeadEjet,"LeadEjet/I");  		// for E rank
    tFmsjet->Branch("uedensity",&muedensity,"uedensity/F");
	 tFmsjet->Branch("ueMul",&mueMul,"ueMul/F");
...
/*
 ...
 if (mcevent!=0){
  cout<<"event generator label: "<<mcevent->eventGeneratorEventLabel()<<endl;
}else {
  cout<<" No McEv         "<<endl;
}

    int nt = mcevent->numberOfPrimaryTracks();
    int reactionpos=0;
    double gangle=0;
    double Zrr=0;

    StSPtrVecMcTrack& track = mcevent->tracks();
//    for(int i=0;i<nt;i++){

	  StMcTrack* mctrack = track[0];
       	 cout<<"geant ID "<<mctrack->geantId()<<endl;
       	 cout<<"pdgID "<<mctrack->pdgId()<<endl;
       	 StParticleDefinition* particle = mctrack->particleDefinition();
       	 cout<<"incident particle "<<particle->name()<<" rapidity "<<mctrack->pseudoRapidity()<<" phi "<<mctrack->momentum().phi()<<" rapidiyt "<<mctrack->momentum().pseudoRapidity()<<endl;

	  StMcVertex *stopv = mctrack->stopVertex();
               if (stopv->numberOfDaughters()!=2){cout<<"daughter !=2    "<<endl;   }
                StMcTrack* gtrack1 = stopv->daughter(0);
             StParticleDefinition* particle1 = mctrack->particleDefinition();
        cout<<"incident particle "<<particle1->name()<<" rapidity "<<gtrack1->pseudoRapidity()<<" phi "<<gtrack1->momentum().phi()<<" rapidiyt "<<gtrack1->momentum().pseudoRapidity()<<"      e1= "<<gtrack1->energy()<<endl;

                StMcTrack* gtrack2 = stopv->daughter(1);
             StParticleDefinition* particle2 = mctrack->particleDefinition();
        cout<<"incident particle "<<particle2->name()<<" rapidity "<<gtrack2->pseudoRapidity()<<" phi "<<gtrack2->momentum().phi()<<" rapidiyt "<<gtrack2->momentum().pseudoRapidity()<<"       e2=     "<<gtrack2->energy()<<endl;
		const TLorentzVector gE1(gtrack1->fourMomentum().px(),gtrack1->fourMomentum().py(),gtrack1->fourMomentum().pz(),gtrack1->fourMomentum().e() );
		const TLorentzVector gE2(gtrack2->fourMomentum().px(),gtrack2->fourMomentum().py(),gtrack2->fourMomentum().pz(),gtrack2->fourMomentum().e() );
		TVector3 gP1(gE1.Px(),gE1.Py(),gE1.Pz());
		TVector3 gP2(gE2.Px(),gE2.Py(),gE2.Pz());
		gangle=gP1.Angle(gP2);
		Zrr = TMath::Abs(gE1.E()-gE2.E())/(gE1.E()+gE2.E());
		cout<<gangle<<"     angle and Zrr from simu        "<<Zrr<<"  "<<gE1.E()<<"   "<<gE2.E()    <<endl;

  //   }

                        StFmsDiPhoton* simudiphoton = new StFmsDiPhoton();
                        simudiphoton->E1 = gE1.E();
                        simudiphoton->E2 = gE2.E();
                        simudiphoton->Eta1 = gE1.PseudoRapidity ();
                        simudiphoton->Eta2 = gE2.PseudoRapidity ();
                        simudiphoton->Phi1 = gE1.Phi();
                        simudiphoton->Phi2 = gE2.Phi();
                        simudiphoton->Ph1id = -1;
                        simudiphoton->Ph2id = -1;
                        simudiphoton->Cluid1 = -1;
                        simudiphoton->Ncluph1 = -1;
                        simudiphoton->Cluid2 = -1;
                        simudiphoton->Ncluph2 = -1;

                        TLorentzVector simulpair = gE1 + gE2;
                        simudiphoton->E12 = simulpair.E();
                        simudiphoton->M12 = simulpair.M();
                        simudiphoton->Z12 = Zrr;
                        simudiphoton->Eta = simulpair.Eta ();
                        simudiphoton->Phi = simulpair.Phi();
			simudiphoton->OpAngle = gangle;
			
			mSimudiphotons->Add(simudiphoton);
...
//  fmstrig->forceRunNumber(11098017);
//  fmstrig->useQTSim();
  assert(fmstrig);

    int FP201 = fmstrig->FP201output();
    Int_t trg1 = 0; //fmstrig->FmsSmallClusterTh0(); //1
    Int_t trg2 = 0 ; //fmstrig->FmsSmallClusterTh1(); //2
    Int_t trg3 = 0; //fmstrig->FmsLargeClusterTh0();
    Int_t trg4 = 0; //fmstrig->FmsLargeClusterTh1();
    Int_t trg5 = 0; //fmstrig->FmsJetPatchTh0();
    Int_t trg6 = 0 ;//fmstrig->FmsJetPatchTh1();
    Int_t trg7 = 0; //fmstrig->FmsLargeClusterTh1();
    Int_t trg8 = 0; //fmstrig->FmsJetPatchTh0();
    Int_t trg9 = 0 ;//fmstrig->FmsJetPatchTh1();
  ...
  fmstrig->Clear();
     ...
     Int_t trg = 1000000 + trg1*100000 + trg2*10000 + trg3*1000 + trg4*100 + trg5*10 + trg6;
               (TrigIds).push_back(trg);        
      cout<<" TRIGGER : SmBS 1 2 LgBS 1 2 JP 1 2   : "<<trg1<<"  "<<trg2<<"  "<<trg3<<"  "<<trg4<<"  "<<trg5<<"  "<<trg6<<"  "<<trg7<<"  "<<trg8<<"  "<<trg9<<"  "<<trg<<" ~F201~ "<<FP201<<endl;
   if(trg9+trg8+trg6+trg5+trg3+trg2==0){ cout<<"--------------skip=============== "<<endl;}// return kStOK; }
//ZZ

*/
	if(stphysics){		//vertex from TPC, only saves the highest (positively) ranked vertex
	...
	else{	//stfms, no tpc vertex, use fit to bbc time bin
...
//
cout<<Period<<"   !!!!!!!!!!!!!!!!!!!!!"<<endl;
if  (Period==1 || Period==2 || Period==3)	{A=0.1432;B=4102;}
if  (Period==4 || Period==5 )	{A=0.1393;B=4098;}
if  (Period==6 || Period==7 || Period==8)	{A=0.1400;B=4101;}
if  (Period==9 )	{A=0.1388;B=4115;}
    if(bbcTimeBin!=0) { mvertexzyuxi= -A*(bbcTimeBin-B);  hVertexZyuxi->Fill(mvertexzyuxi);}//in cm
    if(bbcTimeBin==0)  mvertexzyuxi = -15.7; //in cm
...
//for run15
    ...
    if( tdcMatchEast==1 && tdcMatchWest==1 ) { // BBC slewing correction (east/west)
      ...
      vertexZ = -30.0;
		...
		mvertexz = vertexZ;
 		if(mvertexzyuxi!= -30)hVertexZ->Fill(mvertexz);
 	//	if(mvertexzyuxi!= -36)  hVertexZyuxi->Fill(mvertexzyuxi);
	...
	//	mRnum = Rnum;
		...
		int tofmul2 = 0 ;
            ...
            for(int i=1; i<=16; i++){
                int tac=muDst->event()->triggerData()->bbcTDC(east,i);
                if(tac>100 && tac<2400){
                    int adc=muDst->event()->triggerData()->bbcADC(east,i);
                    if(adc>50) bbcmult++;
                }   
            }   
	...
	//===============deduce trigger type==================//Yuxi 02/07/2011
        ...
        StTriggerId l1trig = trig.nominal();
         ...
         if( l1trig.isTrigger(480823) ) (TrigIds).push_back(480823);        //   FMSSmBS3
...
*/
	...
	if(emc){ //QA part has been moved to StFmsQAMaker
	...
	Float_t ZrrCut = 0.8;
/*	Float_t PtCut = 0.5;  //lower to 0.5 GeV, 
	Float_t EtaCutLow = 2.7;
	Float_t EtaCutHigh = 4.0;
	Float_t JetEtaCutLow = -1.0;
	Float_t JetEtaCutHigh = 2.0;
	Float_t FMSJetEtaCutLow = 2.7;
	Float_t FMSJetEtaCutHigh = 4.0;

	Float_t jetverzCut = 60; //cm	//not used
	Float_t MCutLow = 0.0; //postpone the mass cut
	Float_t MCutHigh = 1.0;	
*/	
	...
	StJetEvent* jetFMS = 0;
	jetFMS = mJetmaker->event("AntiKtR070FMS");
...
//---------------------part1------------------record photon > Ecut GeV    
	...
	for(StFmsClusterIterator iclu = fms->clusters().begin(); iclu != fms->clusters().end(); iclu++){
		...
		for(auto ipts = fmspoints.begin(); ipts != fmspoints.end(); ipts++){
			...
			if(!(Fiducial(php3,pid)) ){			//fiducial volume cut
		      ...
		      if(mphid > 9 )  {   mRx1=phx/3.822-floor(phx/3.822);   mRx2=phy/3.875-floor(phy/3.875);}
       	      //if(phid > 9 )         mChi2S=PHchi2;
              //if(phid <= 9)         mChi2L=PHchi2;

 				//	tmppoint->setRX1((*ipts)->RX1());
				//	tmppoint->setRX2((*ipts)->RX2());
				//	tmppoint->setEnergy((*ipts)->energy());
...
//---------------------part5------------------//pion sorted by energy, and not using same photon 
	const  int ndiphotons = mFmsdiphotons ->GetEntries(); //for corelated diphotons
			...
			Int_t fsortPHE[120];
			int jetCollinsTag[10];
            for(int jJet=0 ; jJet<10 ;  ++jJet){
                jetCollinsTag[jJet]=-1;
            }    
		...
		for(Int_t idiph = 0; idiph < ndiphotons; idiph++){
			...
			if (check==1 ) {//conflit with accepted pions
	...
	//		cout<<" S="<< phiS<<" SY="<< phiSY<<" H=   "<<phiH <<" C=  "<<PhiC<<" yPhiC="<<yPhiC<< endl; 	
			...
			mCorr=1;//correlated
			if(mCorr==1) jetCollinsTag[jettag]=1;
...
//---------------------part6------------------record jet 
        ...
        //  StJetCandidate* jet = jetFMS->jet(iJet);
			...
			//	cout<<"numberOfTracks ()="<<jet->numberOfTracks ()<<"  towers"<<mjetntow<<endl;
            	for(Int_t iph = 0; iph < mjetntow; iph++){
						StJetTower * tower= jet->tower(iph);
				//		cout<<" jet tow E="<<tower->energy()<<"  eta="<<tower->etabin()<<endl ;
            ...
            //  tagPH[ipha]=-1;
                    ...
                    //      double nlgain1= EdepCor(ei,ph1id) ;
                            StLorentzVectorF li = liraw*1;
                            double phE1 =li.e() ;
                            //cout<<" jet#="<<iJet<<" photonE="<<phE1<<endl;
                            if (phE1<PhECut) continue;
                            double phEta = li.pseudoRapidity ();
                            double phPhi = li.phi();
                            int phCluid  = vi->parentClusterId();
                            ...
                            Float_t dphi = jetPhi - phPhi;
                              ...
                              if(dphi<-TMath::Pi()) dphi =  2*TMath::Pi() + dphi;
                                ...
                                Float_t dR   = TMath::Sqrt( dphi*dphi+ deta*deta  );
                  if (dR<0.7)   {tagPH[ipha]=iJet ; ++phcount;   
				  		//cout<<" mymatch " <<ipha<<"  E=   "<<phE1<<endl;
						}

            }//end corelation with photon
            //cout<<" jet#="<<iJet<<" #tower="<<mjetntow<<" #photon="<<phcount<<" jetE="<<jetE<<" jetpt="<<jetpt<<endl;
            mjetnph=phcount;
            	if (jetCollinsTag[iJet]==1) mCollinsTag=1;
            muedensity=jet->uedensity()*jet->area();
            mueMul=jet->ueMul();
                cout<<"UE="<<muedensity<<" "<<mueMul<<" eta="<<jetEta<<" area="<<jet->area()<<" density"<<jet->uedensity()<<endl;

				tFmsjet->Fill();
        }//jet loop end o part6
...
Bool_t StFmsAnalysisMaker::Fiducial(const StThreeVectorF photon,int mod) const {   
	...
	float OutBoundx[4] = { 96.198-2.9, 96.198-2.9, 44.883-1.9,  44.883-1.9 };
	float InBoundx[4]  = {  3.206+2.9,  3.206+2.9,  2.841+1.9,   2.841+1.9 };
	float HiBoundy[4]  = { 95.894-2.9, 95.894-2.9, 44.5625-1.9, 44.5625-1.9};
	float LoBoundy[4]  = {-95.902+2.9,-95.902+2.9,-44.5625+1.9,-44.5625+1.9};
...
//		nlgain = EDepCorrection->Eval(20.)/EDepCorrection->Eval(e0); //large cells are calibrated at 25GeV (sym/asy. decayed eta?..)
	...
	else if(nstb==10 || nstb==11){	//small cells
