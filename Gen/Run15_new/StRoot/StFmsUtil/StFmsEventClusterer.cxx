// $Id: StFmsEventClusterer.cxx,v 1.1 2020/08/01 23:58:30 zwzhu Exp $
//
...
struct TwoPhotonFitParameters {
  ...
  TwoPhotonFitParameters(const std::vector<double>& xyWidth,
    ...
    //ZZ  when small merge to large , x y would keep being large cell width
    //mTowerWidthXY always for large cell, 
    //So
    //1 it would use large cell fit function  
    //2 fit parameter should be changed. .
        double w1=0;
        double w2=0;
 
    if (ww==0)  {w1=x; w2=y;}// could be no merging or large cell cluster when merging, anyway ,nothing need be done
    if (ww==1)  {w1=3.822; w2=3.875;}

//	cout<<"x in FitP "<< x <<"    w1="<<w1<<endl;

    start = std::array<double, 7>{ {
      PH2_START_NPH,
      x * cluster->x(),// both terms are scaled, no need to use w1/w2 
      y * cluster->y(),
      PH2_START_FSIGMAMAX * w1 * cluster->sigmaMax() / 2, // this should be * 1/1.3 if using x when merging
    ...
    lower = std::array<double, 7>{ {
      ...
      std::max(0.1345*2/cluster->energy()*(735-vertexZ) , 0.5*w1 ),
//      std::max(PH2_LOWER_XMAX_F / pow(sigmaMaxE, PH2_LOWER_XMAX_POW), PH2_LOWER_XMAX_LIMIT) * x / 2,
    ...
    upper.at(3) = std::max(upper.at(3), start.at(3) * 3);
...
Int_t StFmsEventClusterer::findClusters() {
  ...
  case 0:
    mClusters.remove_if(IsBadCluster(0.5, BAD_MAX_TOW_LRG));
//    mClusters.remove_if(IsBadCluster(BAD_MIN_E_LRG, BAD_MAX_TOW_LRG));
    break;
  case 1:
    mClusters.remove_if(IsBadCluster(0.5, BAD_MAX_TOW_SML));
