// $Id: StFmsClusterFitter.cxx,v 1.1 2020/08/01 23:58:30 zwzhu Exp $
//
// $Log: StFmsClusterFitter.cxx,v $
// Revision 1.1  2020/08/01 23:58:30  zwzhu
// *** empty log message ***
//
...
int mshowershapewithangle=3;// for static func   //run15 small need narrow shape
int mmerge=0;// for static func
int vertexz=0;
...
void choose_fitParameters(int detID){
  ...
  if (detID<9.1) unused=5;
  if (detID>9.1) unused=3;
  ...
  if ( mshowershapewithangle==3 ) {
    fitParameters= {unused, a11, a12, a13, b11, b12, b13, w1 ,unused, unused,
                                        unused, a21, a22, a23, b21*0.8, b22*0.8, b23*0.8, w2 ,unused, unused,
                                        unused, a31, a32, a33, b31*0.8, b32*0.8, b33*0.8, w3 ,unused, unused,
                                        unused, a41, a42, a43, b41*0.8, b42*0.8, b43*0.8, w4 ,unused, unused,
                                        unused, a51, a52, a53, b51*0.8, b52*0.8, b53*0.8, w5 ,unused, unused,
                                        unused, a61, a62, a63, b61*0.8, b62*0.8, b63*0.8, w6 ,unused, unused          };
}
...
if(mshowershapewithangle==0){
  const double widthx = (parameters[0]>4 ? 5.812: 3.822) ;
  const double widthy = (parameters[0]>4 ? 5.812: 3.875) ;
  for (int ix = 0; ix < 2; ++ix) {
    for (int iy = 0; iy < 2; ++iy) {
      double signX = std::pow(-1., ix);  // 1 or -1
      double signY = std::pow(-1., iy);  // 1 or -1
      std::array<double, 2> s{ {xy[0] + signX * widthx / 2.,    // x +/- d/2
                                xy[1] + signY * widthy / 2.} }; // y +/- d/2
 ...
 if(mshowershapewithangle >0){
      ...
      Double_t Zmax = -999;
        ...
        if (parameters[0]>4) { yoff=98.8; Zmax=735.45; xoff=0.3;
                for(Int_t i = 0; i < 6; i++){
                        Zc[i]=ZcL[i];
                }
        }
        if (parameters[0]<4) { yoff=46.5; Zmax=735.45; xoff=0.93;
                for(Int_t i = 0; i < 6; i++){
                        Zc[i]=ZcS[i];
                }
        }
        ...
        Double_t tany = ( yoff - xy[3]) / (Zmax ); 
        Double_t tanx = ( xy[1]- xoff ) / (Zmax ); //ZZ just in case     
...
void StFmsClusterFitter::setTowers(StFmsTowerCluster::Towers* towers) { 
	...
	if(mshowershapewithangle>0 && mScaleShowerShape>0){
    if(mTowers->front()->hit()->detectorId() <= 9){ 
