...
//theory from cyrill
int BBC[5] = {7300,18000,30000,43500, -14000}
//int BBC[5]= {3000,12000,24000,36000, -14000};
int shift = 850;
...
Double_t cyrillepAu_pt53[27]={8.035558030881161e-6,8.00618604243299e-6,9.272028817197547e-6,0.000012296645433118835,0.00001894784941197356,0.000027957826024474038,0.00004515180398781511,0.00007723509017786493,0.00009814718063916196,0.00011521534548833548,0.00011977791671351406,0.00012089728493668655,0.00012113387104742701,0.00012126629054607242,0.00012113387104742701,0.00012089728493668655,0.00011977791671351406,0.00011521534548833548,0.00009814718063916196,0.00007723509017786493,0.00004515180398781511,0.000027957826024474038,0.00001894784941197356,0.000012296645433118835,9.272028817197547e-6,8.00618604243299e-6,8.035558030881161e-6};
    ...
    TGraph *graphpp41 = new TGraph(27,cyrillepp_dphi,cyrillepp_pt41);
    ...
    TGraph *graphpAu41 = new TGraph(27,cyrillepp_dphi,cyrillepAu_pt41);
...
double PAR0pAu[8],PAR1pAu[8],PAR2pAu[8];
    ...
    f->SetLineColor(0); f->SetLineWidth(0);
...
//Draw theory
     auto theory = new TCanvas("theory","cyrill",10,10,1000,300);
theory->Divide(3);
theory->cd(1);
graphpp21->Draw();
graphpAu21->Draw("same");
graphpAu21->SetLineColor(kBlue);
TLegend *l = new TLegend(0.1,0.7,0.48,0.9);
l->AddEntry(graphpp21,"pp","l");
l->AddEntry(graphpAu21,"pAu","l"); 
l->Draw("same");
theory->cd(2);
graphpp42->Draw();
graphpAu42->Draw("same");
graphpAu42->SetLineColor(kBlue);
theory->cd(3);
graphpp53->Draw();
graphpAu53->Draw("same");
graphpAu53->SetLineColor(kBlue);

cout<<"theory 21 "<<endl;
cout<<PAR1pAu[0]/PAR1pp[0]<<" "<<ratioerr(PAR1pAu[0],PARErr1pAu[0],PAR1pp[0],PARErr1pp[0])<<endl;
cout<<"theory 41 "<<endl;
cout<<PAR1pAu[2]/PAR1pp[2]<<" "<<ratioerr(PAR1pAu[2],PARErr1pAu[2],PAR1pp[2],PARErr1pp[2])<<endl;

shift = 850.;
TGraphErrors* cyrill21 = new TGraphErrors();
        cyrill21->SetPoint(0, BBC[3]-3*shift,PAR1pAu[0]/PAR1pp[0]);
        cyrill21->SetPointError(0, 0.0,ratioerr(PAR1pAu[0],PARErr1pAu[0],PAR1pp[0],PARErr1pp[0]));
/*
        cyrill21->SetPoint(1, BBC[3],PAR1pAu[1]/PAR1pp[1]);
        cyrill21->SetPointError(1, 0.0,ratioerr(PAR1pAu[1],PARErr1pAu[1],PAR1pp[1],PARErr1pp[1]));
        cyrill21->SetPoint(2, BBC[3]+shift,PAR1pAu[2]/PAR1pp[2]);
        cyrill21->SetPointError(2, 0.0,ratioerr(PAR1pAu[2],PARErr1pAu[2],PAR1pp[2],PARErr1pp[2]));
        cyrill21->SetPoint(3, BBC[3]+2*shift,PAR1pAu[3]/PAR1pp[3]);
        cyrill21->SetPointError(3, 0.0,ratioerr(PAR1pAu[3],PARErr1pAu[3],PAR1pp[3],PARErr1pp[3]));
*/
TGraphErrors* cyrill32 = new TGraphErrors();
  //      cyrill32->SetPoint(0, BBC[3]-1*shift,PAR1pAu[4]/PAR1pp[4]);
    //    cyrill32->SetPointError(0, 0.0,ratioerr(PAR1pAu[4],PARErr1pAu[4],PAR1pp[4],PARErr1pp[4]));
        cyrill32->SetPoint(0, BBC[3],PAR1pAu[5]/PAR1pp[5]);
        cyrill32->SetPointError(0, 0.0,ratioerr(PAR1pAu[5],PARErr1pAu[5],PAR1pp[5],PARErr1pp[5]));
      //  cyrill32->SetPoint(2, BBC[3]+1*shift,PAR1pAu[6]/PAR1pp[6]);
        //cyrill32->SetPointError(2, 0.0,ratioerr(PAR1pAu[6],PARErr1pAu[6],PAR1pp[6],PARErr1pp[6]));

TGraphErrors* cyrill53 = new TGraphErrors();
        cyrill53->SetPoint(0, BBC[3]+shift,PAR1pAu[7]/PAR1pp[7]);
        cyrill53->SetPointError(0, 0.0,ratioerr(PAR1pAu[7],PARErr1pAu[7],PAR1pp[7],PARErr1pp[7]));
...
//Data
	...
	double sys32 = 0.061*1.414, sys42 = 0.107*1.414, sys52 = 0.127*1.414;
	double  sys43 = 0.154*1.414, sys53 = 0.186*1.414;
	TGraphErrors* g21 = new TGraphErrors();
	g21->SetPoint(0, BBC[0]-3*shift,RAT1_4[2][1]);
	g21->SetPoint(1, BBC[1]-3*shift,RAT1_5[2][1]);
	g21->SetPoint(2, BBC[2]-3*shift,RAT1_15[2][1]);
	g21->SetPoint(3, BBC[3]-3*shift,RAT1_16[2][1]);
	...
	g21s->SetPoint(0, BBC[0]-shift*3,RAT1_4[2][1]);
	g21s->SetPoint(1, BBC[1]-shift*3,RAT1_5[2][1]);
	g21s->SetPoint(2, BBC[2]-shift*3,RAT1_15[2][1]);
	g21s->SetPoint(3, BBC[3]-shift*3,RAT1_16[2][1]);
	//g21s->SetPoint(4, BBC[4]-shift*1,RAT1_2[2][1]);
	//add total error = sqrt(stat error*stat error + sys error*sys error)
	/*
	g21s->SetPointError(0, 0.0,sqrt(ERROR1_4[2][1]*ERROR1_4[2][1]+sys21*sys21*RAT1_4[2][1]*RAT1_4[2][1]));
	g21s->SetPointError(1, 0.0,sqrt(ERROR1_5[2][1]*ERROR1_5[2][1]+sys21*sys21*RAT1_5[2][1]*RAT1_5[2][1]));
	g21s->SetPointError(2, 0.0,sqrt(ERROR1_15[2][1]*ERROR1_15[2][1]+sys21*sys21*RAT1_15[2][1]*RAT1_15[2][1]));
	g21s->SetPointError(3, 0.0,sqrt(ERROR1_16[2][1]*ERROR1_16[2][1]+sys21*sys21*RAT1_16[2][1]*RAT1_16[2][1]));
*/
	//add sys error
g21s->SetPointError(0, 0.0,sys21*RAT1_4[2][1]);
	...
	TGraphErrors* gpAl21 = new TGraphErrors();
	gpAl21->SetPoint(0, BBCpAl[0]-shift*3,1.05949);
	gpAl21->SetPoint(1, BBCpAl[1]-shift*3,0.722891);
	gpAl21->SetPoint(2, BBCpAl[2]-shift*3,0.664188);
        gpAl21->SetPointError(0, 0.0,0.131172);
	gpAl21->SetPointError(1, 0.0,0.122374);
	gpAl21->SetPointError(2, 0.0,0.138604);
	TGraphErrors* gpAl21s = new TGraphErrors();
	gpAl21s->SetPoint(0, BBCpAl[0]-shift*3,1.05949);
	gpAl21s->SetPoint(1, BBCpAl[1]-shift*3,0.722891);
	gpAl21s->SetPoint(2, BBCpAl[2]-shift*3,0.664188);
        gpAl21s->SetPointError(0, 0.0,0.0299624);
	gpAl21s->SetPointError(1, 0.0,0.0204434);
	gpAl21s->SetPointError(2, 0.0,0.0187832);

TGraphErrors* g31 = new TGraphErrors();
	g31->SetPoint(0, BBC[0]-shift,RAT1_4[3][1]);
	g31->SetPoint(1, BBC[1]-shift,RAT1_5[3][1]);
	g31->SetPoint(2, BBC[2]-shift,RAT1_15[3][1]);
	g31->SetPoint(3, BBC[3]-shift,RAT1_16[3][1]);
	//g31->SetPoint(4, BBC[4],RAT1_2[3][1]);
	...
	//g31->SetPointError(4, 0.0,ERROR1_2[3][1]);
        TGraphErrors* g31s = new TGraphErrors();
	g31s->SetPoint(0, BBC[0]-shift,RAT1_4[3][1]);
	g31s->SetPoint(1, BBC[1]-shift,RAT1_5[3][1]);
	g31s->SetPoint(2, BBC[2]-shift,RAT1_15[3][1]);
	g31s->SetPoint(3, BBC[3]-shift,RAT1_16[3][1]);
	//g31s->SetPoint(4, BBC[4],RAT1_2[3][1]);
/*
	g31s->SetPointError(0, 0.0,sqrt(ERROR1_4[3][1]*ERROR1_4[3][1]+sys31*sys31*RAT1_4[3][1]*RAT1_4[3][1]));
	g31s->SetPointError(1, 0.0,sqrt(ERROR1_5[3][1]*ERROR1_5[3][1]+sys31*sys31*RAT1_5[3][1]*RAT1_5[3][1]));
	g31s->SetPointError(2, 0.0,sqrt(ERROR1_15[3][1]*ERROR1_15[3][1]+sys31*sys31*RAT1_15[3][1]*RAT1_15[3][1]));
	g31s->SetPointError(3, 0.0,sqrt(ERROR1_16[3][1]*ERROR1_16[3][1]+sys31*sys31*RAT1_16[3][1]*RAT1_16[3][1]));
*/
g31s->SetPointError(0, 0.0,sys31*RAT1_4[3][1]);
	...
	//g31s->SetPointError(4, 0.0,sys31*RAT1_2[3][1]);
...
TGraphErrors* g41 = new TGraphErrors();
	...
	//g41->SetPoint(4, BBC[4]+shift,RAT1_2[4][1]);
	...
	//g41->SetPointError(4, 0.0,ERROR1_2[4][1]);
	...
	//g41s->SetPoint(4, BBC[4]+shift,RAT1_2[4][1]);
/*
	g41s->SetPointError(0, 0.0,sqrt(ERROR1_4[4][1]*ERROR1_4[4][1]+sys41*sys41*RAT1_4[4][1]*RAT1_4[4][1]));
	g41s->SetPointError(1, 0.0,sqrt(ERROR1_5[4][1]*ERROR1_5[4][1]+sys41*sys41*RAT1_5[4][1]*RAT1_5[4][1]));
	g41s->SetPointError(2, 0.0,sqrt(ERROR1_15[4][1]*ERROR1_15[4][1]+sys41*sys41*RAT1_15[4][1]*RAT1_15[4][1]));
	g41s->SetPointError(3, 0.0,sqrt(ERROR1_16[4][1]*ERROR1_16[4][1]+sys41*sys41*RAT1_16[4][1]*RAT1_16[4][1]));
*/
	...
	//g41s->SetPointError(4, 0.0,sys41*RAT1_2[4][1]);

TGraphErrors* g51 = new TGraphErrors();
	g51->SetPoint(0, BBC[0]+3*shift,RAT1_4[5][1]);
	g51->SetPoint(1, BBC[1]+3*shift,RAT1_5[5][1]);
	g51->SetPoint(2, BBC[2]+3*shift,RAT1_15[5][1]);
	g51->SetPoint(3, BBC[3]+3*shift,RAT1_16[5][1]);
	//g51->SetPoint(4, BBC[4]+2*shift,RAT1_2[5][1]);
	...
	//g51->SetPointError(4, 0.0,ERROR1_2[5][1]);
	TGraphErrors* g51s = new TGraphErrors();
	g51s->SetPoint(0, BBC[0]+3*shift,RAT1_4[5][1]);
	g51s->SetPoint(1, BBC[1]+3*shift,RAT1_5[5][1]);
	g51s->SetPoint(2, BBC[2]+3*shift,RAT1_15[5][1]);
	g51s->SetPoint(3, BBC[3]+3*shift,RAT1_16[5][1]);
	//g51s->SetPoint(4, BBC[4]+2*shift,RAT1_2[5][1]);
/*
	g51s->SetPointError(0, 0.0,sqrt(ERROR1_4[5][1]*ERROR1_4[5][1]+sys51*sys51*RAT1_4[5][1]*RAT1_4[5][1]));
	g51s->SetPointError(1, 0.0,sqrt(ERROR1_5[5][1]*ERROR1_5[5][1]+sys51*sys51*RAT1_5[5][1]*RAT1_5[5][1]));
	g51s->SetPointError(2, 0.0,sqrt(ERROR1_15[5][1]*ERROR1_15[5][1]+sys51*sys51*RAT1_15[5][1]*RAT1_15[5][1]));
	g51s->SetPointError(3, 0.0,sqrt(ERROR1_16[5][1]*ERROR1_16[5][1]+sys51*sys51*RAT1_16[5][1]*RAT1_16[5][1]));
*/
	...
	//g51s->SetPointError(4, 0.0,sys51*RAT1_2[5][1]);

TGraphErrors* g32 = new TGraphErrors();
	g32->SetPoint(0, BBC[0]-2*shift,RAT1_4[3][2]);
	g32->SetPoint(1, BBC[1]-2*shift,RAT1_5[3][2]);
	g32->SetPoint(2, BBC[2]-2*shift,RAT1_15[3][2]);
	g32->SetPoint(3, BBC[3]-2*shift,RAT1_16[3][2]);
	//g32->SetPoint(4, BBC[4],RAT1_2[3][2]);
	g32->SetPointError(0, 0.0,ERROR1_4[3][2]);
	g32->SetPointError(1, 0.0,ERROR1_5[3][2]);
	g32->SetPointError(2, 0.0,ERROR1_15[3][2]);
	g32->SetPointError(3, 0.0,ERROR1_16[3][2]);
	//g32->SetPointError(4, 0.0,ERROR1_2[3][2]);
TGraphErrors* g32s = new TGraphErrors();
	g32s->SetPoint(0, BBC[0]-2*shift,RAT1_4[3][2]);
	g32s->SetPoint(1, BBC[1]-2*shift,RAT1_5[3][2]);
	g32s->SetPoint(2, BBC[2]-2*shift,RAT1_15[3][2]);
	g32s->SetPoint(3, BBC[3]-2*shift,RAT1_16[3][2]);
	//g32s->SetPoint(4, BBC[4],RAT1_2[3][2]);
/*
	g32s->SetPointError(0, 0.0,sqrt(ERROR1_4[3][2]*ERROR1_4[3][2]+sys32*sys32*RAT1_4[3][2]*RAT1_4[3][2]));
	g32s->SetPointError(1, 0.0,sqrt(ERROR1_5[3][2]*ERROR1_5[3][2]+sys32*sys32*RAT1_5[3][2]*RAT1_5[3][2]));
	g32s->SetPointError(2, 0.0,sqrt(ERROR1_15[3][2]*ERROR1_15[3][2]+sys32*sys32*RAT1_15[3][2]*RAT1_15[3][2]));
	g32s->SetPointError(3, 0.0,sqrt(ERROR1_16[3][2]*ERROR1_16[3][2]+sys32*sys32*RAT1_16[3][2]*RAT1_16[3][2]));
*/
	g32s->SetPointError(0, 0.0,sys32*RAT1_4[3][2]);
	g32s->SetPointError(1, 0.0,sys32*RAT1_5[3][2]);
	g32s->SetPointError(2, 0.0,sys32*RAT1_15[3][2]);
	g32s->SetPointError(3, 0.0,sys32*RAT1_16[3][2]);
	//g32s->SetPointError(4, 0.0,sys32*RAT1_2[3][2]);
	
TGraphErrors* g42 = new TGraphErrors();
	g42->SetPoint(0, BBC[0],RAT1_4[4][2]);
	g42->SetPoint(1, BBC[1],RAT1_5[4][2]);
	g42->SetPoint(2, BBC[2],RAT1_15[4][2]);
	g42->SetPoint(3, BBC[3],RAT1_16[4][2]);
	//g42->SetPoint(4, BBC[4]+shift,RAT1_2[4][2]);
	g42->SetPointError(0, 0.0,ERROR1_4[4][2]);
	g42->SetPointError(1, 0.0,ERROR1_5[4][2]);
	g42->SetPointError(2, 0.0,ERROR1_15[4][2]);
	g42->SetPointError(3, 0.0,ERROR1_16[4][2]);
	//g42->SetPointError(4, 0.0,ERROR1_2[4][2]);
 TGraphErrors* g42s = new TGraphErrors();
	g42s->SetPoint(0, BBC[0],RAT1_4[4][2]);
	g42s->SetPoint(1, BBC[1],RAT1_5[4][2]);
	g42s->SetPoint(2, BBC[2],RAT1_15[4][2]);
	g42s->SetPoint(3, BBC[3],RAT1_16[4][2]);
	//g42s->SetPoint(4, BBC[4]+shift,RAT1_2[4][2]);
/*
	g42s->SetPointError(0, 0.0,sqrt(ERROR1_4[4][2]*ERROR1_4[4][2]+sys42*sys42*RAT1_4[4][2]*RAT1_4[4][2]));
	g42s->SetPointError(1, 0.0,sqrt(ERROR1_5[4][2]*ERROR1_5[4][2]+sys42*sys42*RAT1_5[4][2]*RAT1_5[4][2]));
	g42s->SetPointError(2, 0.0,sqrt(ERROR1_15[4][2]*ERROR1_15[4][2]+sys42*sys42*RAT1_15[4][2]*RAT1_15[4][2]));
	g42s->SetPointError(3, 0.0,sqrt(ERROR1_16[4][2]*ERROR1_16[4][2]+sys42*sys42*RAT1_16[4][2]*RAT1_16[4][2]));
*/
	g42s->SetPointError(0, 0.0,sys42*RAT1_4[4][2]);
	g42s->SetPointError(1, 0.0,sys42*RAT1_5[4][2]);
	g42s->SetPointError(2, 0.0,sys42*RAT1_15[4][2]);
	g42s->SetPointError(3, 0.0,sys42*RAT1_16[4][2]);
	//g42s->SetPointError(4, 0.0,sys42*RAT1_2[4][2]);
	
TGraphErrors* g52 = new TGraphErrors();
	g52->SetPoint(0, BBC[0]+2*shift,RAT1_4[5][2]);
	g52->SetPoint(1, BBC[1]+2*shift,RAT1_5[5][2]);
	g52->SetPoint(2, BBC[2]+2*shift,RAT1_15[5][2]);
	g52->SetPoint(3, BBC[3]+2*shift,RAT1_16[5][2]);
	//g52->SetPoint(4, BBC[4]+2*shift,RAT1_2[5][2]);
	g52->SetPointError(0, 0.0,ERROR1_4[5][2]);
	g52->SetPointError(1, 0.0,ERROR1_5[5][2]);
	g52->SetPointError(2, 0.0,ERROR1_15[5][2]);
	g52->SetPointError(3, 0.0,ERROR1_16[5][2]);
	//g52->SetPointError(4, 0.0,ERROR1_2[5][2]);
TGraphErrors* g52s = new TGraphErrors();
	g52s->SetPoint(0, BBC[0]+2*shift,RAT1_4[5][2]);
	g52s->SetPoint(1, BBC[1]+2*shift,RAT1_5[5][2]);
	g52s->SetPoint(2, BBC[2]+2*shift,RAT1_15[5][2]);
	g52s->SetPoint(3, BBC[3]+2*shift,RAT1_16[5][2]);
	//g52s->SetPoint(4, BBC[4]+2*shift,RAT1_2[5][2]);
/*
		g52s->SetPointError(0, 0.0,sqrt(ERROR1_4[5][2]*ERROR1_4[5][2]+sys52*sys52*RAT1_4[5][2]*RAT1_4[5][2]));
	g52s->SetPointError(1, 0.0,sqrt(ERROR1_5[5][2]*ERROR1_5[5][2]+sys52*sys52*RAT1_5[5][2]*RAT1_5[5][2]));
	g52s->SetPointError(2, 0.0,sqrt(ERROR1_15[5][2]*ERROR1_15[5][2]+sys52*sys52*RAT1_15[5][2]*RAT1_15[5][2]));
	g52s->SetPointError(3, 0.0,sqrt(ERROR1_16[5][2]*ERROR1_16[5][2]+sys52*sys52*RAT1_16[5][2]*RAT1_16[5][2]));
*/
	g52s->SetPointError(0, 0.0,sys52*RAT1_4[5][2]);
	g52s->SetPointError(1, 0.0,sys52*RAT1_5[5][2]);
	g52s->SetPointError(2, 0.0,sys52*RAT1_15[5][2]);
	g52s->SetPointError(3, 0.0,sys52*RAT1_16[5][2]);
	//g52s->SetPointError(4, 0.0,sys52*RAT1_2[5][2]);
	

TGraphErrors* g43 = new TGraphErrors();
	g43->SetPoint(0, BBC[0]-shift,RAT1_4[4][3]);
	g43->SetPoint(1, BBC[1]-shift,RAT1_5[4][3]);
	g43->SetPoint(2, BBC[2]-shift,RAT1_15[4][3]);
	g43->SetPoint(3, BBC[3]-shift,RAT1_16[4][3]);
	g43->SetPointError(0, 0.0,ERROR1_4[4][3]);
	g43->SetPointError(1, 0.0,ERROR1_5[4][3]);
	g43->SetPointError(2, 0.0,ERROR1_15[4][3]);
	g43->SetPointError(3, 0.0,ERROR1_16[4][3]);
TGraphErrors* g43s = new TGraphErrors();
	g43s->SetPoint(0, BBC[0]-shift,RAT1_4[4][3]);
	g43s->SetPoint(1, BBC[1]-shift,RAT1_5[4][3]);
	g43s->SetPoint(2, BBC[2]-shift,RAT1_15[4][3]);
	g43s->SetPoint(3, BBC[3]-shift,RAT1_16[4][3]);
	g43s->SetPointError(0, 0.0,sys43*RAT1_4[4][3]);
	g43s->SetPointError(1, 0.0,sys43*RAT1_5[4][3]);
	g43s->SetPointError(2, 0.0,sys43*RAT1_15[4][3]);
	g43s->SetPointError(3, 0.0,sys43*RAT1_16[4][3]);
	//g53s->SetPointError(4, 0.0,sys53*RAT1_2[5][3]);
	
TGraphErrors* g53 = new TGraphErrors();
	g53->SetPoint(0, BBC[0]+shift,RAT1_4[5][3]);
	g53->SetPoint(1, BBC[1]+shift,RAT1_5[5][3]);
	g53->SetPoint(2, BBC[2]+shift,RAT1_15[5][3]);
	g53->SetPoint(3, BBC[3]+shift,RAT1_16[5][3]);
	//g53->SetPoint(4, BBC[4]+2*shift,RAT1_2[5][3]);
	g53->SetPointError(0, 0.0,ERROR1_4[5][3]);
	g53->SetPointError(1, 0.0,ERROR1_5[5][3]);
	g53->SetPointError(2, 0.0,ERROR1_15[5][3]);
	g53->SetPointError(3, 0.0,ERROR1_16[5][3]);
	//g53->SetPointError(4, 0.0,ERROR1_2[5][3]);
TGraphErrors* g53s = new TGraphErrors();
	g53s->SetPoint(0, BBC[0]+shift,RAT1_4[5][3]);
	g53s->SetPoint(1, BBC[1]+shift,RAT1_5[5][3]);
	g53s->SetPoint(2, BBC[2]+shift,RAT1_15[5][3]);
	g53s->SetPoint(3, BBC[3]+shift,RAT1_16[5][3]);
	//g53s->SetPoint(4, BBC[4]+2*shift,RAT1_2[5][3]);
/*
			g53s->SetPointError(0, 0.0,sqrt(ERROR1_4[5][3]*ERROR1_4[5][3]+sys53*sys53*RAT1_4[5][3]*RAT1_4[5][3]));
	g53s->SetPointError(1, 0.0,sqrt(ERROR1_5[5][3]*ERROR1_5[5][3]+sys53*sys53*RAT1_5[5][3]*RAT1_5[5][3]));
	g53s->SetPointError(2, 0.0,sqrt(ERROR1_15[5][3]*ERROR1_15[5][3]+sys53*sys53*RAT1_15[5][3]*RAT1_15[5][3]));
	g53s->SetPointError(3, 0.0,sqrt(ERROR1_16[5][3]*ERROR1_16[5][3]+sys53*sys53*RAT1_16[5][3]*RAT1_16[5][3]));
*/
	g53s->SetPointError(0, 0.0,sys53*RAT1_4[5][3]);
	g53s->SetPointError(1, 0.0,sys53*RAT1_5[5][3]);
	g53s->SetPointError(2, 0.0,sys53*RAT1_15[5][3]);
	g53s->SetPointError(3, 0.0,sys53*RAT1_16[5][3]);
	//g53s->SetPointError(4, 0.0,sys53*RAT1_2[5][3]);
	

TGraphErrors* g54 = new TGraphErrors();
	g54->SetPoint(0, BBC[0],RAT1_4[5][4]);
	g54->SetPoint(1, BBC[1],RAT1_5[5][4]);
	g54->SetPoint(2, BBC[2],RAT1_15[5][4]);
	g54->SetPoint(3, BBC[3],RAT1_16[5][4]);
	g54->SetPointError(0, 0.0,ERROR1_4[5][4]);
	g54->SetPointError(1, 0.0,ERROR1_5[5][4]);
	g54->SetPointError(2, 0.0,ERROR1_15[5][4]);
	g54->SetPointError(3, 0.0,ERROR1_16[5][4]);

	TF1 *baseline = new TF1("baseline","1",1000,52000);
	baseline->GetYaxis()->SetRangeUser(-0.6,2.6);
	...
	pad_1->SetMargin(0.11, 0.01, 0.00, 1.1);
	...
	baseline->Draw();
	g21->GetYaxis()->SetRangeUser(-0.1,2.5);
	g21->SetMarkerColor(kMagenta-6);
	gpAl21->SetMarkerColor(1);
	gpAl21->SetMarkerStyle(30);
	gpAl21->SetLineColor(1);
	gpAl21->SetLineWidth(width);
	gpAl21->SetMarkerSize(size);
	...
	gpAl21s->SetLineWidth(width);
	gpAl21s->SetLineColor(1);
	gpAl21s->SetMarkerSize(size);
	//gpAl21->Draw("samePZ");
	//gpAl21s->Draw("same[]");
	g21s->Draw("same[]");
	...
	g51s->Draw("same[]");
	cyrill21->SetMarkerSize(size-0.1);
        cyrill21->SetLineWidth(width+0.5);
	cyrill21->SetLineColor(46);
        cyrill21->SetMarkerStyle(8);
        //cyrill21->Draw("3csame");
        //cyrill21->Draw("samePZ");
        cyrill21->SetFillColorAlpha(46,0.05);
	cyrill21->SetMarkerColor(46);

        double xx = 0.67;
        double yy = 0.09;
        drawLatex(0.15, 0.8,"#sqrt{s_{NN}} = 200 GeV, 2.6 < #eta < 4.0",62,0.088,kBlack);
        drawLatex(0.18, 0.58,"p^{trig}_{T}",62,0.085,kBlack);
        drawLatex(xx, yy-0.015,"p^{asso}_{T} = 1.0-1.5 GeV/c",62,0.08,kBlack);
 	...
 	TLegend *leg1_3 = new TLegend(0.14,-0.03,0.36,0.17);	
        leg1_3->AddEntry(cyrill21,"rcBK, b=0","p");
        leg1_3->SetFillStyle(0);
        leg1_3->SetLineColor(0);
	leg1_3->SetFillColorAlpha( kWhite, 0);
	leg1_3->SetBorderSize ( 0 );
	//leg1_3->Draw("same");
	...
	g32->GetYaxis()->SetTitleOffset(0.46);
	...
	baseline2->SetLineWidth(1.5);
        baseline2->GetYaxis()->SetRangeUser(0.1,1.3);
	baseline2->GetYaxis()->SetTitle("Relative area");
...
cyrill32->SetMarkerSize(size-0.1);
cyrill32->SetLineColor(46);
cyrill32->SetFillColorAlpha(46,0.05);
cyrill32->SetFillStyle(3001);
//        cyrill32->Draw("3csame");
//        cyrill32->Draw("samePZ");
        cyrill32->SetMarkerStyle(8);
	cyrill32->SetMarkerColor(46);
	...
	g52s->Draw("same[]");
	drawLatex(xx, yy,"p^{asso}_{T} = 1.5-2.0 GeV/c",62,0.09,kBlack);
	...
	pad_3->cd();
        ...
        baseline3->GetYaxis()->SetRangeUser(0,1.9);
	...
	cyrill53->SetMarkerSize(size-0.1);
	cyrill53->SetMarkerStyle(8);
	cyrill53->SetMarkerColor(46);
	//cyrill53->Draw("samePZ");
...
g43s->SetLineColor(kCyan+2);
	...
	g43->Draw("samePZ");
	g43s->Draw("same[]");
	drawLatex(xx, yy+0.09,"p^{asso}_{T} = 2.0-2.5 GeV/c",62,0.08,kBlack);
	//drawLatex(xx-0.07, yy-0.08,"MiniBias",42,0.085,46);
        TLegend *leg3 = new TLegend(0.1,0.7,0.48,0.9);
        leg3->AddEntry(g53,"pT1 = 2.8-5.0 GeV/c","P");

	//TOF Mult
	
        //3000<bbce<12000: 24.79
        //12000<bbce<24000:32.74
        //24000<bbce<36000:41.42
        //bbce>36000:57.83
        /*
...
drawTof(5700, 0.13,"18.4%",62,0.06,13);
	...
	c1->SaveAs("Area_pppAu_5.pdf");


//Draw MB data, only one point without BBCE selection, pT1=1.0-1.4,pT2=0.5-1.0
/*
	//Draw inclusive pi0 yeilds accordting to BBCE
       TArrow *line = new TArrow(1000,0.1,45000,0.1,0.01);
	line->SetAngle(60);
	...
	g54->Draw("samePZ");
*/
c1->SaveAs("Area_pppAu.pdf");
cout<<"2.5-3.0, 1.0-1.5"<<endl;
g41->Print();
g41s->Print();
cout<<"2.5-3.0, 1.5-2.0"<<endl;
g42->Print();
g42s->Print();
cout<<"2.5-3.0, 2.0-2.5"<<endl;
g43->Print();
g43s->Print();
cout<<"theory 2.5-3.0, 1.0-1.5"<<endl;
cyrill21->Print();
cyrill32->Print();
cyrill53->Print();
cout<<"3.0-5.0, 1.0-1.5"<<endl;
g51->Print();
g51s->Print();
cout<<"2.5-3.0, 1.5-2.0"<<endl;
g52->Print();
